...
Die App liegt im Unterverzeichnis `stadt-zuerich-next/`; npm-Skripte (z.B.
`npm run validate:prozesse`, `npm run check:regression`) werden von dort aus
ausgeführt.
...
- **Kanonisch** ist `docs/process-data-contract.md` und das JSON Schema
  `stadt-zuerich-next/schemas/opengov-process-schema.json`. tessera gleicht sich
  darauf ab; bei Abweichung gilt **dieses** Repo.
- Prozess-JSONs liegen unter `stadt-zuerich-next/data/prozesse/<city>/<id>.json`
